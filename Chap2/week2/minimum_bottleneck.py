...
class dijkstra:
    ...
    def heap_Based_dijkstra(self, adjacent_list, source):
        ...
        return A
    def greedy(self, adjacent_list, source):
    ...
    def main(self, file='dijkstraData.txt',source=1, end=None):
        ...
        toc = time.time()
        print("O(m + n^2),array based dijkstra algorithm takes: "+str("%.6f"%(toc - tic))+"s")

        tic = time.time()
        A = self.heap_Based_dijkstra(adjacent_list, source - 1)
        toc = time.time()
        print("O(m + n * log(n),heap based dijkstra algorithm takes: "+str("%.6f"%(toc - tic))+"s")

        for i in range(len(end)):
            res.append(A[end[i] - 1])

        A = self.greedy(adjacent_list, source -1 )
